"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.TransformGQLTags = exports.BabelGQLWebpackPlugin = exports.QueryManager = void 0;
const query_manager_1 = require("./query-manager");
Object.defineProperty(exports, "QueryManager", { enumerable: true, get: function () { return query_manager_1.QueryManager; } });
Object.defineProperty(exports, "BabelGQLWebpackPlugin", { enumerable: true, get: function () { return query_manager_1.BabelGQLWebpackPlugin; } });
/**
 * https://stackoverflow.com/a/52231746/153718
 */
function areWeTestingWithJest() {
    return process.env.JEST_WORKER_ID !== undefined;
}
function parseTag(t, qm, path, removeQuery) {
    var _a;
    const gqlString = path.node.quasi.quasis
        .map((q) => q.value.cooked)
        .join("")
        .trim();
    let parsed;
    try {
        parsed = qm.parseGraphQL(gqlString);
    }
    catch (error) {
        throw path.buildCodeFrameError((_a = "GraphQL: " + error.message) !== null && _a !== void 0 ? _a : "Unknown graphql parsing error");
    }
    return recursiveObjectExpression(t, {
        queries: parsed.queries.map((query) => ({
            query: removeQuery ? "" : query.query,
            queryName: query.queryName,
            queryId: query.queryId,
            usedFragments: query.usedFragments,
        })),
        fragments: parsed.fragments.map((fragment) => ({
            fragment: removeQuery ? "" : fragment.fragment,
            fragmentName: fragment.fragmentName,
            fragmentId: fragment.fragmentId,
            usedFragments: fragment.usedFragments,
        })),
    });
}
const externalModules = [
    {
        // import gql from 'graphql-tag'
        name: "graphql-tag",
    },
    {
        name: "graphql-tag.macro",
    },
    {
        // import { graphql } from 'gatsby'
        name: "gatsby",
        identifier: "graphql",
    },
    {
        name: "apollo-server-express",
        identifier: "gql",
    },
    {
        name: "apollo-server",
        identifier: "gql",
    },
    {
        name: "react-relay",
        identifier: "graphql",
    },
    {
        name: "apollo-boost",
        identifier: "gql",
    },
    {
        name: "apollo-server-koa",
        identifier: "gql",
    },
    {
        name: "apollo-server-hapi",
        identifier: "gql",
    },
    {
        name: "apollo-server-fastify",
        identifier: "gql",
    },
    {
        name: " apollo-server-lambda",
        identifier: "gql",
    },
    {
        name: "apollo-server-micro",
        identifier: "gql",
    },
    {
        name: "apollo-server-azure-functions",
        identifier: "gql",
    },
    {
        name: "apollo-server-cloud-functions",
        identifier: "gql",
    },
    {
        name: "apollo-server-cloudflare",
        identifier: "gql",
    },
];
function findIdentifier(importName) {
    return externalModules.find((m) => m.name === importName);
}
function recursiveObjectExpression(t, ob) {
    if (typeof ob === "string") {
        return t.stringLiteral(ob);
    }
    if (typeof ob === "boolean") {
        return t.booleanLiteral(ob);
    }
    if (ob === null) {
        return t.nullLiteral();
    }
    if (Array.isArray(ob)) {
        return t.arrayExpression(ob.map((item) => recursiveObjectExpression(t, item)));
    }
    if (typeof ob === "number") {
        return t.numericLiteral(ob);
    }
    if (!ob) {
        return t.nullLiteral();
    }
    return t.objectExpression(Object.keys(ob)
        .sort()
        .map((key) => {
        return t.objectProperty(t.identifier(key), recursiveObjectExpression(t, ob[key]));
    }));
}
function initFileState() {
    return {
        ourName: null,
        externalTags: new Set(),
        inject: [],
    };
}
class TransformGQLTags {
    constructor(options) {
        this.fileState = initFileState();
        this.qm = options.qm;
        this.babel = options.babel;
    }
    resetFileState() {
        this.fileState = initFileState();
    }
    asBabelPlugin() {
        const t = this.babel.types;
        const isProduction = Boolean(process.env.NODE_ENV === "production");
        return {
            visitor: {
                Program: {
                    enter: (path) => {
                        // Reset import name state when entering a new file
                        this.resetFileState();
                    },
                    exit: (path, state) => {
                        if (!this.fileState.ourName) {
                            return;
                        }
                        for (const graphqlObject of this.fileState.inject) {
                            path.pushContainer("body", t.callExpression(t.identifier(this.fileState.ourName), [graphqlObject]));
                        }
                    },
                },
                ImportDeclaration: (path, state) => {
                    const opts = state.opts || {};
                    const targets = new Set(["babel-gql", "@headup/wp", opts === null || opts === void 0 ? void 0 : opts.ownModuleName].filter(Boolean));
                    const importName = path.node.source.value;
                    if (targets.has(importName)) {
                        for (const s of path.node.specifiers) {
                            if (!t.isImportSpecifier(s)) {
                                continue;
                            }
                            if (s.imported.name === "gql") {
                                this.fileState.ourName = s.local.name;
                            }
                        }
                    }
                    const external = findIdentifier(importName);
                    if (external) {
                        for (const s of path.node.specifiers) {
                            if (!t.isImportSpecifier(s)) {
                                continue;
                            }
                            if (external.identifier &&
                                s.imported.name === external.identifier) {
                                this.fileState.externalTags.add(s.local.name);
                            }
                        }
                    }
                },
                TaggedTemplateExpression: (path, state) => {
                    var _a, _b;
                    if (!path.node.loc) {
                        return;
                    }
                    const tag = path.node.tag;
                    if (!t.isIdentifier(tag)) {
                        return;
                    }
                    const removeQuery = (_b = (_a = state.opts) === null || _a === void 0 ? void 0 : _a.removeQuery) !== null && _b !== void 0 ? _b : isProduction;
                    if (this.fileState.ourName &&
                        tag.name === this.fileState.ourName) {
                        const graphqObject = parseTag(t, this.qm, path, removeQuery);
                        path.replaceWith(t.callExpression(t.identifier(this.fileState.ourName), [graphqObject]));
                    }
                    else if (this.fileState.externalTags.has(tag.name)) {
                        if (this.fileState.inject) {
                            this.fileState.inject.push(parseTag(t, this.qm, path, removeQuery));
                        }
                    }
                },
            },
        };
    }
}
exports.TransformGQLTags = TransformGQLTags;
function babelGQLPlugin(babel) {
    query_manager_1.debug("Initializing Babel plugin");
    // prettier-ignore
    if (!query_manager_1.BABEL_GQL_GLOBAL.babelGQLWebpackPlugin && !areWeTestingWithJest()) {
        console.error();
        console.error("############################################################");
        console.error("#                                                          #");
        console.error("#  ERROR: BabelGQLWebpackPlugin is not configured!         #");
        console.error("#  See https://github.com/valu-digital/babel-gql#install   #");
        console.error("#                                                          #");
        console.error("############################################################");
        console.error();
    }
    const qm = new query_manager_1.QueryManager();
    qm.registerAsGlobal();
    const transform = new TransformGQLTags({
        qm,
        babel,
    });
    return transform.asBabelPlugin();
}
exports.default = babelGQLPlugin;
//# sourceMappingURL=plugin.js.map