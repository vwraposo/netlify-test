...
const validatePassword_1 = require("../../../../sdk/auth/Service/validatePassword");
...
const EmailAndPassword = ({ variant: v, returnUrl, }) => {
    const [state, dispatch] = react_1.useReducer(state_1.reducer, { state: 'signInForm' });
    ...
    return state.state.startsWith('emailForm') ? (react_1.default.createElement(EmailForm_1.default, { state: state, variant: variant, onBack: () => dispatch({ type: 'signInForm' }), onSubmit: (email) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        }) })) : state.state.startsWith('signUpForm') ? (react_1.default.createElement(SignUpForm_1.default, { state: state, variant: variant, onBack: () => dispatch({ type: 'signInForm' }), onSubmit: (code, password, confirmPassword) => __awaiter(void 0, void 0, void 0, function* () {
            if (!validate_1.isValidAccessCode(code)) {
                dispatch({ type: 'signUpForm.invalidAccessCodeInput' });
            }
            if (!validate_1.isValidPassword(password)) {
        ...
        }) })) : (react_1.default.createElement(SignInForm_1.default, { state: state, variant: variant, onSubmit: (email, password) => __awaiter(void 0, void 0, void 0, function* () {
            if (!validate_1.isValidEmail(email)) {
            ...
            try {
                yield startLogin({ user: email });
                yield validatePassword_1.validatePassword({ login: email, password });
                onLoginSuccessful(returnUrl);
            }
            catch (_b) {
                dispatch({ type: 'signInForm.authError' });
            }
        }), onForgotPassword: () => dispatch({ type: 'emailForm' }), onSignUp: () => dispatch({ type: 'emailForm' }) }));
};
exports.default = EmailAndPassword;
