  ...
  for (var _iterator = _rollupPluginBabelHelpers._createForOfIteratorHelperLoose(groups), _step; !(_step = _iterator()).done;) {
    ...
    for (var i = 0; i < maxLength; i += 1) {
      ...
      if (!item || angular && item.disabled) {
        var isFrist = i === 0;
        var previousItem = isFrist && angular ? getCurrentId.findFirstEnabledItem(group) : group[i - 1];
...
function reducer(state, action) {
  ...
  switch (action.type) {
    ...
    case "unregisterItem":
      {
        ...
        if (currentId && currentId === _id2) {
          var nextId = getCurrentId.getCurrentId(_rollupPluginBabelHelpers._objectSpread2(_rollupPluginBabelHelpers._objectSpread2({}, _nextState), {}, {
    ...
    case "down":
      {
        ...
        var verticalItems = verticalizeItems(reverse.flatten(fillGroups(reverse.groupItems(items), currentId, angular)));
    ...
    case "up":
      {
        var _verticalItems = verticalizeItems(reverse.reverse(reverse.flatten(fillGroups(reverse.groupItems(items), currentId, angular)))); // If currentId is initially set to null, we'll always focus the
...
function useCompositeState(initialState) {
  ...
  var _useSealedState = useSealedState.useSealedState(initialState),
      ...
      sealed = _rollupPluginBabelHelpers._objectWithoutPropertiesLoose(_useSealedState, ["unstable_virtual", "rtl", "orientation", "currentId", "loop", "wrap", "unstable_angular"]);
  ...
  var idState = Id_IdState.unstable_useIdState(sealed); // register/unregister may be called when this component is unmounted. We
